...
async def send_rules(callback: types.CallbackQuery):
    ...
    await callback.message.answer_video(
        ...
        caption=(
            ...
            f"2. Купить электронный билет, стоимостью\n"
            f"{TICKET_PRICE} руб и получить личный номер участника🔢\n\n"
            f"<i>Всего в продаже {available_tickets} билетов, количество\n"
            "мест ограничено</i>😎\n\n"
    ...
    try:
        ...
        if member.status in ["member", "administrator", "creator"]:
            ...
            await callback.message.answer(
                ...
                "Осталось заполнить анкету, приобрести билет и ты в деле⬇️",
                reply_markup=inline_kb
            )
        else:
...
async def process_phone(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"<b>Напиши сколько билетов ТЫ хочешь приобрести?☺️</b>\n\n"
                         f"⚠️ Осталось: {available_tickets} билетов\n\n"
                         f"Стоимость 1 билета - <b>{TICKET_PRICE} руб</b>💸\n\n"
                         f"Чем больше у тебя билетов, тем больше шансов выиграть автомобиль. "
                         f"В связи с этим мы подготовили спец. предложение <b><i>3 "
                         f"билета по цене 2-х</i></b>🤩\n\n"
                         f"<i>*спец. предложение будет действовать первые 5 часов с запуска продаж</i>")
