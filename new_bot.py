...
async def send_welcome(message: types.Message):
    ...
    if arg == "payment_done":
        await message.answer(
            ...
            "Ты участвуешь в промо-розыгрыше, удачи! 🍀\n\n"
            "Твои код(ы) были отправлены в предыдущем сообщении. Сохрани их!"
        )
        return
    ...
    await message.answer(
        ...
        "<b><i>Но это еще не все!!!</i></b> Мы подготовили для вас\n"
        "еще несколько приятных призов:\n\n"
        "💫 <b><i>3 покупателя наших фирменных наклеек\n"
        "получат - </i></b>Наушники Apple AirPods 4\n"
        "💫 <b><i>2 покупателя получат - </i></b>Часы Apple Watch\n"
        "SE 2\n"
        "💫 <b><i>2 покупателя получат - </i></b>Умную колонку\n"
        "Алиса мини\n"
        "💫 <b><i>1 покупатель получит - </i></b>Прогулку на катере\n"
        "по центру Санкут-Петербурга\n"
...
async def send_rules(callback: types.CallbackQuery):
    ...
    await callback.message.answer_video(
        ...
        caption=(
            "<b><i>Разыгрывать автомобиль мы будем среди\n"
            "покупателей наших наклеек 😁 Каждому из\n"
            "которых будет присвоен персональный код</i></b>\n"
            "1️⃣2️⃣3️⃣\n\n"
            "<b><i>Условия участия просты:📋</i></b>\n\n"
            "1. Быть подписанным на наш канал <b><i>NPAuto</i></b>\n"
            "🔥 чтоб быть в курсе событий\n\n"
            f"2. Купить наклейку стоимостью\n"
            f"{TICKET_PRICE} руб и получить свой персональный\n"
            "код🔢\n\n"
            f"<i>Всего в продаже {available_tickets} наклеек, купив которые,\n"
            "Вы можете участвовать в акции😎/n/n"
            "*Промо-розыгрыш проводиться до\n"
            "31.07.2025. Как получить свой приз мы\n"
            "расскажем в нашем ТГ канале!"
        ),
...
async def process_phone(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"<b>Напиши сколько наклеек ТЫ хочешь приобрести?😊</b>\n\n"
                         ...
                         f"В связи с этим мы подготовили спец. предложение:\n"
                         f"<b><i>купив 2 наклейки, ты получаешь в подарок\n"
...
async def send_success_message(user_id: int, ticket_numbers: list[str]):
    ...
    await bot.send_message(
        user_id,
        f"🎉 <b>Оплата подтверждена! Поздравляем, ты</b>\n\n"
        f"🎉 <b>участвуешь в промо-розыгрыше, удачи!🍀</b>\n\n"
        f"Ты приобрел(а) <b>{len(ticket_numbers)}</b> наклейки\n"
        f"Твой код(ы): <b>{', '.join(ticket_numbers)}</b>\n\n"
        f"Сохрани свой(и) код(ы)! Именно <b>по коду мы</b>\n"
        f"<b>определим обладателя Audi A4 и других</b>\n"
        f"<b>призов 💫</b>\n\n"
        f"Как и где можно получить свою наклейку, мы\n"
        f"расскажем в нашем ТГ канале 😎\n\n"
        f"Будь на связи 📲"
    )
